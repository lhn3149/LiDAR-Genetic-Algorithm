...
while (generation < lastgen)
    ...
    while (i< totalPopulation+1)
        D = 100*N*40*d_opt ; 
        N_x = population(:,i);
        [I_ff_norm, Angle_ff] = OPA_farfield(N, N_x, D, k, phi);
        [pks, locs] = findpeaks(I_ff_norm, Angle_ff, "MinPeakHeight", 0.7);
        if isempty(locs)
           FOV = 0; 
        else
            locs_diff = diff(locs);
...
%             f2 = figure('visible',not(figsaveFlag));
%             plot(Angle_ff, I_ff_norm);
%             xlabel("Angle (deg)");
%             ylabel("I");
%             title(titleplot);
%             fplotsave = fname + "\plots\FOV";
%             fig_name1 = sprintf("Genetic_Algoithm_N_%i_gen_%i_iter_%i",N,generation,i);
%             if FOV > 120
%                 fplotsave = fname + "\plots\FOV\very_high_FOV";
%             end
...
fig_name1 = sprintf("Genetic_Algoithm_N_%i_gen_%i_(%i)",N,lastgen,randi(100));
x = 1:1:lastgen;
plot(x, bestFOV);
xlabel("Generations");
ylabel("FOV (deg)");
